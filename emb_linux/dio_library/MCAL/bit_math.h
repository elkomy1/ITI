...
#define ASSIGN_HIGH_NIB(REG,VALUE)    REG=((REG<<4)>>4)|(VALUE<<4)    
// Assign VALUE to low nibble REG 
#define ASSIGN_LOW_NIB(REG,VALUE)     REG=((REG>>4)<<4)|(VALUE) 
...
#define CRSHFT_REG(REG, NO)           REG=(REG<<(8-NO))|(REG>>NO)
// Circular left shift by NO in REG
#define CLSHFT_REG(REG,NO)            REG=(REG>>(8-NO))|(REG<<NO)   
